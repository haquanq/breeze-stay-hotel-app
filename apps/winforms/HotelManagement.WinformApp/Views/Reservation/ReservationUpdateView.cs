...
{
    public partial class ReservationUpdateView : Form
    {
        ...
        public ReservationUpdateView(string selectedReservationId)
        {
            InitializeComponent();
        ...
        private void UpdateDepartureTime()
        {
            try
            {
                ...
                DashboardView._self.LoadRoomStatusAndReservation();
                this.Close();
            }
            catch (Exception ex)
            {
        ...
        {
            if (grvAvailableRoom.CurrentRow == null) return;
            var f = new RoomDetailView(grvAvailableRoom.CurrentRow.Cells["aid"].Value.ToString());
            f.ShowDialog();
